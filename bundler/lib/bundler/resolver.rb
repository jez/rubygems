...
module Bundler
  ...
  class Resolver
    ...
    def raise_not_possible!(package)
      level = @gem_version_promoter.level
      name = package.name
      locked_version = package.locked_version
      requirement = package.dependency

      raise GemNotFound, "#{name} is locked to #{locked_version}, while Gemfile is requesting #{requirement}. --strict --#{level} was specified, but there are no #{level} level upgrades from #{locked_version} satisfying #{requirement}, so version solving has failed"
    end
    ...
    def prepare_dependencies(requirements, packages)
      to_dependency_hash(requirements, packages).map do |dep_package, dep_constraint|
        ...
        if versions.empty? && select_all_versions(dep_package, dep_range).any?
          raise_not_possible!(dep_package)
        end
